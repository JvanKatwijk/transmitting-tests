...
void	hackrfHandler::set_frequency	(uint64_t newFrequency) {
int	res;
...
void	hackrfHandler::set_txvgaValue	(int newGain) {
int	res;
	if ((newGain <= 47) && (newGain >= 0)) {
	   res	= hackrf_set_txvga_gain (theDevice, newGain);
	   if (res != HACKRF_SUCCESS) {
	      fprintf (stderr, "Problem with hackrf_txvga_gain :\n");
	      fprintf (stderr, "%s \n",
	                 this -> hackrf_error_name (hackrf_error (res)));
	      return;
	   }
...
bool	hackrfHandler::start_transmitting (uint64_t frequency) {
int	res;
...
void	hackrfHandler::stop_transmitting	(void) {
int	res;
