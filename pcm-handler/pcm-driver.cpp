...
#include	"pcm-driver.h"
	...
	pcmDriver::pcmDriver (int port, std::string address) {
	   this	-> port		= port;
	   this	-> address	= address;
	   buffer	= new RingBuffer<std::complex<float>> (4 * 32768);
	   connected. store (false);
	   threadHandle	= std::thread (&pcmDriver::run, this);
	   running. store (true);
}

	pcmDriver::~pcmDriver (void) {
...
void	pcmDriver::sendSample (std::complex<float> datum) {
