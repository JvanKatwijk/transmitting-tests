...
struct timeval  tv;
	...
	return ((int64_t)tv. tv_sec * 1000000 + (int64_t)tv. tv_usec);
		...
		fmStreamer::fmStreamer (int		inRate,
	                                int		outRate,
	                                outputDriver	*theGenerator,
	                                RingBuffer<floatSample> *pcmBuffer,
...
void	fmStreamer::stop (void) {
	running. store (false);
}

void	fmStreamer::start	(void) {
	if (running. load ())
	   return;
	threadHandle	= std::thread (&fmStreamer::run, this);
}

void	fmStreamer::run (void) {
...
void	fmStreamer::modulateData (float *bo, int amount, int channels) {
int	i;
	for (i = 0; i < amount; i ++) {
...
uint16_t fmStreamer::rds_crc (uint16_t in) {
int i;
uint16_t reg = 0;
...
void	fmStreamer::rds_bits_to_values (char *out,
	                                 uint16_t in, int len) {
int n	= len;
uint16_t mask;
...
void	fmStreamer::rds_serialize (struct rds_group_s *group, char flags) {
int n = 4;
static
...
//
//
void	fmStreamer::rds_init_groups (struct rds_info_s *info) {
...
void	fmStreamer::rds_group_0A_update (void) {
...
void	fmStreamer::rds_group_2A_update (void) {
static int rt_pos = 0;
static int b_pos = 0;
...
void	fmStreamer::rds_group_3A_update (void) {
static int toggle = 1;
...
void	fmStreamer::rds_group_8A_update (void) {
}

struct rds_group_s *fmStreamer::rds_group_schedule (void) {
static int ps = 1;
