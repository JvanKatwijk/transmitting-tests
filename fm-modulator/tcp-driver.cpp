...
#include	"tcp-driver.h"
	...
	tcpDriver::tcpDriver (int port, std::string address) {
	   this	-> port		= port;
	   this	-> address	= address;
	   buffer	= new RingBuffer<std::complex<float>> (4 * 32768);
	   connected. store (false);
	   threadHandle	= std::thread (&tcpDriver::run, this);
	   running. store (true);
}

	tcpDriver::~tcpDriver (void) {
...
void	tcpDriver::sendSample (std::complex<float> datum) {
