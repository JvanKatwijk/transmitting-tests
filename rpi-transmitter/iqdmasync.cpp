...
iqdmasync::iqdmasync (uint64_t TuneFrequency,
	              uint32_t SR,
	              int Channel,
	              uint32_t FifoSize,
	              int Mode):
...
//	Note : Spurious are at +/-(19.2MHZ/2^20)*Div*N : (N=1,2,3...),
//	So we need to have a big div to spurious away BUT
//	Spurious are ALSO at +/-(19.2MHZ/2^20)*(2^20-Div)*N
//	Max spurious avoid is to be in the center!
//	Theory shoud be that spurious are set away at 19.2/2= 9.6Mhz !
//	But need to get account of div of PLLClock
}

	iqdmasync::~iqdmasync (void) {
...
void	iqdmasync::SetDmaAlgo (void) {
...
//	Order of DMA CS influence maximum rate :
//	here 0,2,1 is the best : why !!!!!!
...
//@3 Delay
	   SetEasyCB (cbp, samplecnt * registerbysample,
	                              syncwithpwm ? dma_pwm : dma_pcm, 1);
...
void	iqdmasync::SetIQSample (uint32_t Index,
	                        std::complex<float> sample, int Harmonic) {

	Index	= Index % buffersize;	
	mydsp. pushsample (sample);

/*	if (mydsp. frequency > 2250)
 *	   mydsp. frequency = 2250;
 *	if (mydsp. frequency < 1000)
 *	   mydsp. frequency = 1000;
 */
//	Frequency
	sampletab [Index * registerbysample] =
	     (0x5A << 24) | GetMasterFrac (mydsp. frequency / Harmonic);
	int IntAmplitude = (int)(mydsp. amplitude * 8.0) - 1;
//	Fixme 1e4 seems to work with SSB
//	but should be an issue with classical IQ file 
	...
	sampletab [Index * registerbysample + 1] = (0x5A<<24) +
	            (IntAmplitudePAD & 0x7) + (1<<4) + (0<<3); // Amplitude PAD

//	sampletab [Index * registerbysample + 2] =
//	            (Originfsel & ~(7 << 12)) | (4 << 12); //Alternate is CLK
	if (IntAmplitude == -1) {
	   sampletab [Index * registerbysample + 2] =
	            (Originfsel & ~(7 << 12)) | (0 << 12);
//	Pin is in -> Amplitude 0
 	}
	else {
	   sampletab [Index * registerbysample + 2] =
	            (Originfsel & ~(7 << 12)) | (4 << 12);
//Alternate is CLK : Fixme : do not work with clk2
	}
...
void	iqdmasync::SetFreqAmplitudeSample (uint32_t Index,
	                                   std::complex<float> sample,
	                                   int Harmonic) {

	Index	= Index % buffersize;	
	sampletab [Index * registerbysample] =
	           (0x5A << 24) | GetMasterFrac (sample.real()/Harmonic);
//Frequency
	...
	sampletab [Index * registerbysample + 1] =
	         (0x5A << 24) + (IntAmplitudePAD & 0x7) +
	                               (1 << 4) + (0<<3); // Amplitude PAD
...
void	iqdmasync::SetIQSamples (std::complex<float> *sample,
	                         size_t Size, int Harmonic = 1) {
size_t NbWritten	= 0;
...
//	   printf("Available before=%d\n",Available);		
	   ...
	   if (TimeToSleep > 0) {
//	      dbg_printf (1,
//	                  "buffer size %d Available %d SampleRate %d Sleep %d\n",
...
//	      sched_yield ();
	   ...
	   if (debug > 0) {	
	      ...
	      if (time_difference < 0)
	         time_difference += 1E9;
//	      dbg_printf (1,
//	                  "Available %d Measure samplerate=%d\n",
//	                  GetBufferAvailable (),
//	                  (int)((GetBufferAvailable()-Available)*1e9/time_difference));
	   ...
	   int ToWrite	= ((int)Size-(int)NbWritten) <
	                           Available ? Size-NbWritten : Available;
...
//	                                    Available, TimeToSleep, ToWrite);		
	   ...
	   if (ModeIQ == MODE_FREQ_A) {
	      for (int i = 0; i < ToWrite; i++) {
	         SetFreqAmplitudeSample (Index + i,
	                                 sample [NbWritten++],
	                                 Harmonic);
	      }
	   }
