...
// Again some functions taken gracefully from F4GKR
//: https://github.com/f4gkr/RadiantBee

//	Normalize to [-180,180):
inline
double	dsp::constrainAngle (double x){
...
// convert to [-360,360]
inline
double	dsp::angleConv (double angle){
	return fmod (constrainAngle (angle), 2 * M_PI);
}

inline
double	dsp::angleDiff (double a, double b){
...
inline
double	dsp::unwrap (double previousAngle, double newAngle){
	return previousAngle - angleDiff (newAngle, angleConv (previousAngle));
}

// Should be replaced with fast_atan2 from rtl_fm
int	dsp::arctan2 (int y, int x) {
int abs_y	= abs(y);
int angle;
