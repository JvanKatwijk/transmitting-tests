...
// Fixme! Need to check to be 256 Aligned for UserMem
	...
	uint32_t MemoryRequired =
	            CBSize*sizeof(dma_cb_t)+UserMemSize*sizeof(uint32_t);
	int NumPages = (MemoryRequired / PAGE_SIZE) + 1;
	dbg_printf (1, "%d Size NUM PAGES %d PAGE_SIZE %d\n",
	                   MemoryRequired, NumPages, PAGE_SIZE);
	mbox. mem_ref = mem_alloc (mbox. handle,
	                           NumPages* PAGE_SIZE, PAGE_SIZE, mem_flag);
//	TODO: How do we know that succeeded? */
	dbg_printf (1, "mem_ref %x\n", mbox.mem_ref);
	mbox.bus_addr = mem_lock (mbox. handle, mbox. mem_ref);
 	dbg_printf (1, "bus_addr = %x\n", mbox. bus_addr);
	mbox. virt_addr =
	   (uint8_t *)mapmem(BUS_TO_PHYS(mbox.bus_addr), NumPages* PAGE_SIZE);
	dbg_printf (1, "virt_addr %p\n", mbox.virt_addr);
	virtbase = (uint8_t *)((uint32_t *)mbox. virt_addr);
	dbg_printf (1,"virtbase %p\n", virtbase);
//	We place DMA Control Blocks (CB) at beginning of virtual memory
	cbarray = (dma_cb_t *)virtbase;
	dbg_printf (1, "cbarray %p\n", cbarray);
//	user memory is placed after
	usermem = (unsigned int *)(virtbase + CBSize * sizeof (dma_cb_t));
	dbg_printf (1, "usermem %p\n", usermem);

	dma_reg. gpioreg [DMA_CS + channel * 0x40] =
	                BCM2708_DMA_RESET|DMA_CS_INT; // Remove int flag
	usleep (100);
	dma_reg. gpioreg [DMA_CONBLK_AD + channel * 0x40] =
	          mem_virt_to_phys((void*)cbarray ); // reset to beginning 
...
//	reset to beginning 
	dma_reg. gpioreg [DMA_CONBLK_AD+channel*0x40] =
	              mem_virt_to_phys ((void*)cbarray );
//	clear debug error flags
	dma_reg. gpioreg [DMA_DEBUG + channel * 0x40] = 7;
	usleep (100);
	dma_reg. gpioreg [DMA_CS+channel*0x40] =
	                             DMA_CS_PRIORITY (7) |
	                             DMA_CS_PANIC_PRIORITY(7) |
	                             DMA_CS_DISDEBUG |DMA_CS_ACTIVE;
	Started = true;
	return 0;
}
...
int	dma::stop (void) {
	dma_reg. gpioreg [DMA_CS + channel * 0x40] = BCM2708_DMA_RESET;
	usleep (1000);
	dma_reg. gpioreg [DMA_CS + channel * 0x40] = BCM2708_DMA_INT |
	                                             BCM2708_DMA_END;
	usleep (100);
	dma_reg. gpioreg [DMA_CONBLK_AD + channel * 0x40] =
	                       mem_virt_to_phys ((void *)cbarray );
...
int	dma::getcbposition (void) {
volatile uint32_t dmacb =
	    (uint32_t)(dma_reg. gpioreg [DMA_CONBLK_AD + channel * 0x40]);
//	dbg_printf (1,"cb=%x\n", dmacb);
	if (dmacb > 0)
...
bool	dma::isrunning (void) {
	return ((dma_reg.gpioreg [DMA_CS + channel * 0x40] & DMA_CS_ACTIVE)>0);
}

bool	dma::isunderflow (void) {
//	if ((dma_reg. gpioreg [DMA_CS + channel * 0x40] & DMA_CS_INT) > 0)
//	   dbg_printf (1,"Status:%x\n",dma_reg.gpioreg[DMA_CS+channel*0x40]);
	return ((dma_reg.gpioreg [DMA_CS + channel * 0x40] & DMA_CS_INT) > 0);
}

bool	dma::SetCB (dma_cb_t *cbp,
	            uint32_t dma_flag,
	            uint32_t src,
	            uint32_t dst, uint32_t repeat) {
	cbp -> info	= dma_flag;
	cbp -> src	= src;
	cbp -> dst	= dst;
	cbp -> length	= 4 * repeat;
...
bool	dma::SetEasyCB (dma_cb_t *cbp,
	                uint32_t index,
	                dma_common_reg dst,
	                uint32_t repeat) {
...
//********************* BUFFER DMA *****************************************
	...
	bufferdma::bufferdma (int Channel,
	                      uint32_t tbuffersize,
	                      uint32_t tcbbysample,
	                      uint32_t tregisterbysample):
	                            dma (Channel,
	                                 tbuffersize * tcbbysample,
	                                 tbuffersize * tregisterbysample) {
	buffersize	= tbuffersize;
	cbbysample	= tcbbysample;
	registerbysample=tregisterbysample;
	dbg_printf (1, "BufferSize %d , cb %d user %d\n",
	            buffersize,
	            buffersize * cbbysample,
...
void	bufferdma::SetDmaAlgo (void) {
}

int bufferdma::GetBufferAvailable (void) {	
int diffsample	= 0;
...
int	bufferdma::GetUserMemIndex (void) {
