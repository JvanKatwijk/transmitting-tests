...
uint32_t gpio::GetPeripheralBase (void) {
...
//******************* DMA Registers *************************************
	...
	dmagpio::dmagpio (void):
	   gpio (GetPeripheralBase () + DMA_BASE, DMA_LEN) {
}

// ***************** CLK Registers *****************************************
	clkgpio::clkgpio (void):
	   gpio (GetPeripheralBase () + CLK_BASE, CLK_LEN) {
	SetppmFromNTP ();
	padgpio level;
	...
	clkgpio::~clkgpio (void) {
	gpioreg [GPCLK_CNTL] = 0x5A000000 |
	                       (Mash << 9) |
	                       pllnumber |
	                       (0 << 4); //4 is START CLK
//	gpioreg [GPCLK_CNTL_2] = 0x5A000000 |
	                         (Mash << 9) |
	                         pllnumber |
	                         (0 << 4); //4 is START CLK
	usleep (100);
...
uint64_t Freq = 0;
	...
	switch (PllNo) {
	   ...
	   case clk_plla:
	      Freq = XOSC_FREQUENCY *
	                   ((uint64_t)gpioreg[PLLA_CTRL] & 0x3ff) +
	             XOSC_FREQUENCY *
	                   (uint64_t)gpioreg[PLLA_FRAC] / (1 << 20);
	      break;
//	   case clk_pllb:
//	      Freq = XOSC_FREQUENCY*((uint64_t)gpioreg[PLLB_CTRL]&0x3ff) +
//	             XOSC_FREQUENCY*(uint64_t)gpioreg[PLLB_FRAC]/(1<<20);
//	      break;
	   case clk_pllc:
...
uint32_t clkgpio::GetMasterFrac (double Frequency) {
	...
	if (ModulateFromMasterPLL) {
	   double FloatMult = 0;
	   if (PllFixDivider == 1) //Using PDIV thus frequency/2
	      FloatMult = ((double)(CentralFrequency + Frequency)) /
	                  ((double)(XOSC_FREQUENCY*2) * (1 - clk_ppm * 1e-6));
	   else
	      FloatMult =
	         ((double)(CentralFrequency + Frequency)*PllFixDivider) /
...
//	https://github.com/SaucySoliton/PiFmRds/blob/master/src/pi_fm_rds.c
...
//	1 is allowed only for MASH=0
	      if (Frequency * divider < MIN_PLL_RATE)
	         continue; // widest accepted frequency range
	      if (Frequency * divider > MIN_PLL_RATE_USE_PDIV) {
//	By Experiment on Rpi3B
	         continue;
	      }
	      max_int_multiplier =
	      ((int)((double)(Frequency + Bandwidth) * divider * xtal_freq_recip));
	      min_int_multiplier =
	      ((int)((double)(Frequency - Bandwidth) * divider * xtal_freq_recip));
...
//dbg_printf(1,"Warning : cross boundary frequency\n");
	...
	if (best_divider != 0) {
	   ...
	   dbg_printf (1, "Found PLL solution for frequency %4.1fMHz : divider:%d VCO: %4.1fMHz\n",
	               ...
	               (Frequency/1e6) *
	                   ((PllFixDivider==1)?2.0:(double)PllFixDivider));
	   return 0;
	}
	else {
...
double	clkgpio::GetFrequencyResolution (void) {
double res = 0;
	...
	if (ModulateFromMasterPLL) {
	   ...
	   uint32_t FreqDivider = (uint32_t)Freqresult;
	   res = (Pllfrequency / (double)(FreqDivider + 1) -
	                Pllfrequency / (double)(FreqDivider)) / 4096.0;
	}
	return res;
}

double	clkgpio::GetRealFrequency (double Frequency) {
double FloatMult = ((double)(CentralFrequency + Frequency) *
	                     PllFixDivider) / (double)(XOSC_FREQUENCY);
uint32_t freqctl = FloatMult * ((double)(1 << 20));
int IntMultiply	= freqctl >> 20; // Need to be calculated to have a center frequency
	freqctl &= 0xFFFFF;		 // Fractionnal is 20bits
	uint32_t FracMultiply = freqctl & 0xFFFFF;
